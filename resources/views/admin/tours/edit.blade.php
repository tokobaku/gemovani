@php /** @var \App\Tour\Tour $tour */ @endphp
...
@section('content')
    <div class="Form">
        ...
        <form action="{{ url("/admin/tours/{$tour->id}") }}" method="POST" id="tour-form">
            @csrf
            @method('PATCH')
            ...
            <div class="Form-FormGroup">
                ...
                <input class="Form-Input" id="url_key" name="url_key" value="{{ old('url_key') ?? $tour->url_key }}">
            </div>
            <div class="Form-FormGroup">
                @error('cover_image')
                <div class="Form-ErrorMessage">{{ $message }}</div>
                ...
                <input class="Form-Input" id="selected-image" name="cover_image" type="hidden" value="{{ old('cover_image') ?? $tour->cover_image }}">
            ...
            <div class="Form-FormGroup">
                ...
                <input type="date" id="start-date" name="start_date" value="{{ old('start_date') ?? $tour->start_date }}">
            </div>
            <div class="Form-FormGroup">
                @error('end_date')
                <div class="Form-ErrorMessage">{{ $message }}</div>
                ...
                <input type="date" id="end-date" name="end_date" value="{{ old('end_date') ?? $tour->end_date }}">
            </div>
            <div class="Form-FormGroup">
                    ...
                    <div class="Expandable-Content">
                        ...
                        <div class="Form-FormGroup">
                            ...
                            <textarea class="Form-Input ToTinyMCE" id="translations_{{ $language['code'] }}_description"
                                      name="translations[{{ $language['code'] }}][description]">{{ old("translations.{$language['code']}.description") ?? @$tour->getTranslation($language['code'])->description }}</textarea>
                        </div>
                    </div>
                </div>
                @endforeach
